...
class LocalSearch:
    ...
    def generate_neighborhood(x=[], neighborhood=[], current_span=[]):
        if len(x) == 1:
            ...
            new_span.append(x[0])
            neighborhood = LocalSearch.generate_neighborhood(x=x[1:], neighborhood=neighborhood, current_span=new_span)
            new_span[-1] = new_span[-1] - LocalSearch.delta
            neighborhood = LocalSearch.generate_neighborhood(x=x[1:], neighborhood=neighborhood, current_span=new_span)
            new_span[-1] = new_span[-1] + 2 * LocalSearch.delta
            neighborhood = LocalSearch.generate_neighborhood(x=x[1:], neighborhood=neighborhood, current_span=new_span)
        return neighborhood
    ...
    def search(self):
        ...
        while it < self.max_iter:
            ...
            neighborhood_search_space = LocalSearch.generate_neighborhood(curr_sol.x)
...
def main():
    ...
    #FIRST ITERATION VALUE OF INITIAL SOLUTION IS MISSING!
    delta = 0.1
    max_iter = 500
    objectives = [f1]
